...
integer i,j,k,kb,kc,k200,k500,k850,k5km
...
do k=1,nzm
  ...
  do j=1,ny
    do i=1,nx
     ...
     qn_xy(i,j) = qn_xy(i,j)+(qcl(i,j,k)+qci(i,j,k))*coef1 !CMuller: add non precip condensate
     qp_xy(i,j) = qp_xy(i,j)+(qpl(i,j,k)+qpi(i,j,k))*coef1 !CMuller: add precip condensate
    mse_xy(i,j)=mse_xy(i,j)+(tabs(i,j,k)+gamaz(k)+fac_cond*qv(i,j,k))*coef1 !CMuller: add MSE
...
do k = 1,nzm
   if(z(k).lt.5000.) then
       k5km = k+1 !first atmospheric level above 5km
   end if
end do
...
do j=1,ny
 do i=1,nx
  ...
  u5km_xy(i,j) = u5km_xy(i,j) + u(i,j,k5km)*dtfactor  
  v5km_xy(i,j) = v5km_xy(i,j) + v(i,j,k5km)*dtfactor  
  w5km_xy(i,j) = w5km_xy(i,j) + w(i,j,k5km)*dtfactor
  qv5km_xy(i,j)= qv5km_xy(i,j)+ qv(i,j,k5km)*dtfactor  
  qn5km_xy(i,j)= qn5km_xy(i,j)+(qcl(i,j,k5km)+qci(i,j,k5km))*dtfactor
  qp5km_xy(i,j)= qp5km_xy(i,j)+(qpl(i,j,k5km)+qpi(i,j,k5km))*dtfactor
  ta5km_xy(i,j)= ta5km_xy(i,j)+ tabs(i,j,k5km)*dtfactor  
 end do
end do
