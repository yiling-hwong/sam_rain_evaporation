...
nfields= 15 !YLH(changed from 23) !CMuller removed&replaced some outputs
if(.not.dolongwave) nfields = nfields-4
if(.not.doshortwave) nfields = nfields-4
...
if(docloud) then
   ...
   nfields1=nfields1+1 !CMuller: add
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=qn_xy(i,j)*coef
       qn_xy(i,j) = 0.
     end do
   end do
  name='IntQN'
  long_name='Non-precip condensate Path'
  units='mm'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
   ...
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=qp_xy(i,j)*coef
       qp_xy(i,j) = 0.
     end do
   end do
  name='IntQP'
  long_name='Precip condensate Path'
  units='mm'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
...
!                               save2Dbin,dompi,rank,nsubdomains)
   ...
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=cp*mse_xy(i,j)*coef/1000./1000.
       mse_xy(i,j) = 0.
     end do
   end do
  name='MSE'
  long_name='Moist Static Energy (x1e-6)'
  units='1e6J/m2'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
...
!   nfields1=nfields1+1 !CMuller: remove (not saved anyway)
!   do j=1,ny
!    do i=1,nx
...
!                               save2Dbin,dompi,rank,nsubdomains)
 ...
 !  do j=1,ny
 !    do i=1,nx
 !      tmp(i,j,1)=prec_inst(i,j)*dz/dt*86400 ! CMuller&lBack fix instantaneous
 !      prec calc
 !    end do
 !  end do
 ! name='PRECI'
...
!  ! cloud top height !CMuller: remove
!   nfields1=nfields1+1
!   do j=1,ny
!      do i=1,nx
...
!   ! cloud top temperature !CMuller: remove
!   nfields1=nfields1+1
!   do j=1,ny
!      do i=1,nx
