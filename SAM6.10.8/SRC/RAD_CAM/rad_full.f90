subroutine rad_full()
  ...
  if(compute_reffc) then
    rel_rad(1:nx,1:ny,1:nzm) = rel_rad(1:nx,1:ny,1:nzm) + Get_reffc() * qcl(1:nx,1:ny,1:nzm) 
  end if
  if(compute_reffi) then
    rei_rad(1:nx,1:ny,1:nzm) = rei_rad(1:nx,1:ny,1:nzm) + Get_reffi() * qci(1:nx,1:ny,1:nzm) 
  end if
  ...
  if(nstep.eq.1.or.nradsteps.ge.nrad) then 
        ...
        ! Default reff computation:
	      ...
	      coszrs = coszrs_in(1) ! needed for the isccp simulator
           ...
           do k=1,nzm
              ...
              qradlw(i,j,m)=qrl(1,k)
              qradsw(i,j,m)=qrs(1,k)
     ...
     if(doradlwhomo) then
        ...
        do k=1,nzm
           ...
           do j=1,ny
            do i=1,nx
             qradlw(i,j,k) = qradz(k)
            end do
           end do
        end do
     end if !end if doradlwhomo

     !Homogenize SW radiation:
     if(doradswhomo) then
        factor = 1./dble(nx*ny)
        do k=1,nzm
           qradz(k) = 0.
        ...
        do k=1,nzm
           ...
           do j=1,ny
            do i=1,nx
             qradsw(i,j,k) = qradz(k)
            end do
           end do
        end do
     end if !end if doradswhomo
   ...
   ! Homogenize radiation:
     if(doradhomo) then    
        ...
        !homogenize the corresponding 2D outputs
        if(doradhomo.or.doradlwhomo) then
        factor = 1./dble(nx*ny)
        buffer1(:) = 0. 
        do j=1,ny
         do i=1,nx
          buffer1(1)=buffer1(1)+ lwnsxy(i,j)
          buffer1(2)=buffer1(2)+ lwntxy(i,j)
          buffer1(3)=buffer1(3)+ lwntmxy(i,j)
          buffer1(4)=buffer1(4)+ lwnscxy(i,j) 
          buffer1(5)=buffer1(5)+ lwntcxy(i,j)
          buffer1(6)=buffer1(6)+ lwdsxy(i,j) 
         end do
        end do
        buffer1(:) = buffer1(:) * factor 
        if(dompi) call task_sum_real8(buffer1,buffer2,6)
        factor = 1./float(nsubdomains)
        lwnsxy(:,:)  = buffer2(1) * factor 
        lwntxy(:,:)  = buffer2(2) * factor 
        lwntmxy(:,:) = buffer2(3) * factor 
        lwnscxy(:,:) = buffer2(4) * factor 
        lwntcxy(:,:) = buffer2(5) * factor 
        lwdsxy(:,:)  = buffer2(6) * factor 
        end if
      
        !homogenize the corresponding 2D outputs
        if(doradhomo.or.doradswhomo) then
        factor = 1./dble(nx*ny)
        buffer1(:) = 0.
        do j=1,ny
         do i=1,nx
          buffer1(1)=buffer1(1)+ swnsxy(i,j)
          buffer1(2)=buffer1(2)+ swntxy(i,j)
          buffer1(3)=buffer1(3)+ swntmxy(i,j)
          buffer1(4)=buffer1(4)+ swnscxy(i,j)
          buffer1(5)=buffer1(5)+ swntcxy(i,j)
          buffer1(6)=buffer1(6)+ swdsxy(i,j)
         end do
        end do
        buffer1(:) = buffer1(:) * factor
        if(dompi) call task_sum_real8(buffer1,buffer2,6)
        factor = 1./float(nsubdomains)
        swnsxy(:,:)  = buffer2(1) * factor
        swntxy(:,:)  = buffer2(2) * factor
        swntmxy(:,:) = buffer2(3) * factor
        swnscxy(:,:) = buffer2(4) * factor
        swntcxy(:,:) = buffer2(5) * factor
        swdsxy(:,:)  = buffer2(6) * factor
        end if
