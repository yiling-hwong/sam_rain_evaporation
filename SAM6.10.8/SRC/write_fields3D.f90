...
!CMuller :remove sponge layer from 3d files (to make it lighter)
use vars
use rad, only: qrad
use params
...
!real(4) tmp(nx,ny,nzm_cm) !CMuller: rm sponge layer
!character *12 c_z(nzm),c_p(nzm),c_dx, c_dy, c_time !CMuller: rm sponge layer
character *12  c_dx, c_dy, c_time
real(4), allocatable :: tmp(:,:,:)
character *12, allocatable :: c_z(:),c_p(:)

!CMuller : rm sponge layer ->
real damp_depth           !CMuller: rm sponge layer: damp_depth is the damping depth as a fraction of the domain height (see damping.f90)
parameter(damp_depth=0.3) !CMuller: rm sponge layer: damp_depth is the damping depth as a fraction of the domain height (see damping.f90)
integer  n_damp, nzm_cm   
do k=nzm,1,-1
 if(z(nzm)-z(k).lt.damp_depth*z(nzm)) then 
   n_damp=nzm-k+1
 endif
end do
nzm_cm=nzm-n_damp-1 !CMuller: last level before enter sponge at k=nzm-n_damp (see damping.f90)
allocate(tmp(nx,ny,nzm_cm), c_z(nzm_cm), c_p(nzm_cm))
!CMuller: rm sponge layer <-


nfields=7 ! number of 3D fields to save
...
if(masterproc.or.output_sep) then
  ...
  if(masterproc) then
    ...
    if(save3Dbin) then
      ...
      write(46) nx,ny,nzm_cm,nsubdomains,nsubdomains_x,nsubdomains_y,nfields
      do k=1,nzm_cm
        write(46) z(k) 
      end do
      do k=1,nzm_cm
      ...
      write(long_name,'(8i4)') nx,ny,nzm_cm,nsubdomains, &
      ...
      write(46) c_time,c_dx,c_dy, (c_z(k),k=1,nzm_cm),(c_p(k),k=1,nzm_cm)
  ...
  units='m/s'
  call compress3D(tmp,nx,ny,nzm_cm,name,long_name,units, &
...
!  nfields1=nfields1+1 !CMuller :remove
!  do k=1,nzm_cm
!   do j=1,ny
!    do i=1,nx
!      tmp(i,j,k)=p(i,j,k)
...
if(compute_reffc.and.(dolongwave.or.doshortwave).and.rad3Dout) then
  nfields1=nfields1+1
  tmp(1:nx,1:ny,1:nzm_cm)=Get_reffc()
  name='REL'
...
if(compute_reffi.and.(dolongwave.or.doshortwave).and.rad3Dout) then
  nfields1=nfields1+1
  tmp(1:nx,1:ny,1:nzm_cm)=Get_reffi()
  name='REI'
...
if(doprecip) then
  ...
  name='QP'
  long_name='Precipitating Water (Rain+Snow+Graupel)' !CMuller :fix name, this actually includes graupel as well.
  units='g/kg'
  call compress3D(tmp,nx,ny,nzm_cm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)
end if
